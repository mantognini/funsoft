...
object ExerciseBuild extends Build {
  ...
  val packageToMoodleTask: Setting[InputTask[Unit]] = packageToMoodle <<= Def.inputTask { (argTask: TaskKey[Seq[String]]) =>
    (argTask, baseDirectory, target, packageOptions, streams, compile in Compile) map { (args: Seq[String], base, outDir, po, s, comp) =>
      ...
      if (args.length > 3 || args.length < 1)
        ...
        gatherSources(out, po, s.cacheDirectory, s.log)
      }
